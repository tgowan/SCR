...
for(i in 1:M){ # Loop over individuals
  for(t in 1:T){ # Loop over primary occasions
    if(z[i,t]==1){ #only simulate if alive (in pop)
      # Pixel containing U location is a multinomial based on distance and habitat covariates
...
LatentState.Mod <- nimbleCode({
  ...
  # space (pixel) use based on distance from possible activity center and habitat covariates in each primary
  pix.pi[1:nPixels,1:nPixels,1:T] <- PixelUse(T=T, distMat=distMat[1:nPixels,1:nPixels], m.alpha=move.alpha, B1=beta1, B2=beta2, B3=beta3, B4=beta4,
                                              SST=sst[1:nPixels,1:T], Depth=depth[1:nPixels])
  ...
  for(i in 1:M) {
    ...
    for (t in 1:T){   #for each primary
      ...
      for (k in 1:K[t]){ #for each secondary
        UPix[i,k,t] ~ dcat(pix.pi[s[i,t],1:nPixels,t])   #pixel
